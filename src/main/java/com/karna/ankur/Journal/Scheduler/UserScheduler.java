...
//       @Scheduled(cron = "0 */2 * * * *")
    ...
    public void fetchUsersAndSendSAMail() {
        ...
        for (UserEntity user : users) {
            ...
            List<Sentiment> sentiments = journalEntries.stream().filter(a -> a.getDate().isAfter(LocalDateTime.now().minus(7, ChronoUnit.DAYS))).map(a -> a.getSentiment()).collect(Collectors.toList());
            ...
            if (mostFrequentSentiment != null) {
                SentimentMessage sentimentMessage = SentimentMessage.builder().email(user.getEmail()).sentiment("This is only for testing purpose. \nMessages are sent my kafka producer and logs are sent to consumer for future reference. \nSentiment for last 7 days " + mostFrequentSentiment).build();
                kafkaTemplate.send("weekly-sentiments",sentimentMessage.getEmail(),sentimentMessage);
